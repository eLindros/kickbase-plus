      ...
      .then((response) => {
        if (response.status === 200) {
          if (response.data.token && response.data.tokenExp) {
            ...
            localStorage.setItem(
              Constants.LOCALSTORAGE.BEARER_TOKEN_EXPIRATION,
  ...
  async loadNextTwoMatchDays() {
    ...
    if (currentMatchDay.no) {
      fetchedGameDays.push({
        m: currentMatchDay.matches,
        md: currentMatchDay.no,
        d: currentMatchDay.no,
      })
      ...
      .catch((errors) => {
        ...
        if (errors.response && errors.response.data) {
          ...
          if (errors.response.data.err === 5050) {
            ...
            store.commit(
              'setErrorMessage',
              "you reached your financial limit (max. 33% debt of your team's market value)"
            )
          }
          if (errors.response.data.err === 5030) {
            ...
            store.commit(
              'setErrorMessage',
              'You have reached the limit for players per team (max. ' +
                store.getters.getSelectedLeague.pl +
                ' players)'
            )
          }
          if (errors.response.data.err === 5070) {
            ...
            store.commit(
              'setErrorMessage',
              'You have reached the limit for players of this Bundesliga team (max. ' +
                store.getters.getSelectedLeague.mpst +
                ' players)'
            )
          }
        }
    ...
    await axios({
      ...
      .catch((errors) => {
        ...
        store.commit(
          ...
          'error occured during putting player on market. lets try it again'
        )
      ...
      .then((response) => {
        if (response.status === 200) {
          if (typeof cb === 'function') {
            store.commit(
              'addLoadingMessage',
              'removed player "' + player.lastName + '" from market'
            )
            cb(response.data)
          }
        }
