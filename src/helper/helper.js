...
const getMarketValueGrowth = (id) => {
  if (
    ...
    store.getters.getPlayers[id].marketValues[
      store.getters.getPlayers[id].marketValues.length - 1
    ] &&
    ...
    return (
      ...
      store.getters.getPlayers[id].marketValues[
        store.getters.getPlayers[id].marketValues.length - 2
      ].m
    )
  }
  return null
...
const smartPlayerStatsLoading = async (playerIds) => {
  store.commit(
    'addLoadingMessage',
    'checking the statistics of the players and loading them if necessary'
  )
  ...
  if (store.getters.getMarketValueComparisonPlayer === null) {
    ...
    const currentMarketValueComparisonPlayerStats = await api.loadPlayersStats(
      store.getters.getMarketValueComparisonPlayer.id,
      true
    )
    if (
      currentMarketValueComparisonPlayerStats.marketValue !==
      store.getters.getMarketValueComparisonPlayer.marketValue
    ) {
      shouldExecuteLoading = true
    }
  }
