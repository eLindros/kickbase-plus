<template>
  ...
  >
    <options-bar info-button-title="your defaults">
      <template #content>
        ...
        <div v-if="computedDeclinableOffers.length">
          ...
          >
            decline all
            <strong>{{ computedDeclinableOffers.length }} offers</strong> that are too low or
            expired
          </v-btn>
      ...
      <template #info>
        <v-alert outlined type="info" :icon="false">
          <div class="d-flex align-center">
            ...
            <p class="caption mb-0">your default settings:</p>
          </div>
          <v-simple-table>
            <template>
              <tbody>
    ...
    <div v-if="loading === false" class="flex-grow-1 options-bar__sibling">
      <v-expansion-panels
        v-model="openPanels"
        multiple
        accordion
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            ...
            >
              ...
              <v-btn block color="grey darken-1 white--text" large @click="putOnMarket(player)"
                >Put on market</v-btn
              >
            </v-card>
          </v-expansion-panel-content>
        </v-expansion-panel>
        <v-expansion-panel v-if="playerWithoutAnyOffer.length > 0">
          <v-expansion-panel-header class="elevation-0">
            <v-icon class="mr-2 player-card-accordion__icon">fa-spinner</v-icon>
            <strong>{{ playerWithoutAnyOffer.length }}</strong> players who have no offer at the
            moment
          </v-expansion-panel-header>
          <v-expansion-panel-content>
            <offer-player-item
      ...
      <offer-player-item
        ...
        @setPlayerOnMarketAgain="setPlayerOnMarketAgain"
      >
      </offer-player-item>
    </div>
...
export default {
  ...
  watch: {
    offers() {
      ...
      if (
        this.getOfferOpenPlayerWithoutAnyOfferPanel === true &&
        this.playerWithoutAnyOffer.length
      ) {
          ...
          if (player.userId && player.userId * 1 === this.getSelf) {
            if (!player.offers || (player.offers && player.offers.length >= 0)) {
              ...
              if (player.hasHighOffer === false && player.hasLowOffer === true) {
                ...
                player.hasExpiredOffers === true &&
                player.hasKickbaseOffersOnly === true
              ) {
