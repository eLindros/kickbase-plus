  ...
  <v-container>
    ...
    <div v-if="getSelectedLeague">
      <table class="tbl">
        <tr>
          ...
          <td class="pa-2 blue-grey darken-1 white--text text-left">
    ...
    <v-list>
      <v-list-item-group v-model="activeLeague" color="primary">
        <v-list-item v-for="(item, i) in getLeagues" :key="i" @click="setLeague(item)">
    ...
    <br />
    <v-textarea name="bearer" label="Bearer Token" :value="getToken" disabled></v-textarea>
  </v-container>
</template>
...
export default {
  ...
  data: () => ({}),
  computed: {
    ...mapGetters(['getBearerToken', 'getLeagues', 'getLeague', 'getSelectedLeague']),
    getToken() {
      return api.getToken()
    },
    activeLeague() {
  ...
  methods: {
    ...mapMutations(['setLoading', 'setLoadingMessages']),
    setLeague(league) {
